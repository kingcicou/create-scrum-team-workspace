...
| 签核前反复改共享仓库 user.name/email | 并发覆盖身份，后续普通提交也可能串号 | signoff CLI 使用命令级身份，不修改仓库配置 |
| 多人同时在单 worktree 签核 | 争抢 index.lock、提交混入或失败 | signoff 写命令使用仓库互斥锁并自动等待 |
...
角色签核是普通文档提交的受控例外：`tools/signoff.mjs sign` 从
`roles.config.json` 为**本次提交**注入姓名和邮箱，不修改 local/global/worktree
配置；Event 首次提交后任何修改都会失效。它解决署名串号，不是身份认证；
高保证场景仍需独立平台账号或签名提交。
