...
```bash
bash <(curl -fsSL https://raw.githubusercontent.com/kingcicou/create-scrum-team-workspace/v0.9.4/create.sh) my-project
```

可叠加任意 CLI 选项：

```bash
bash <(curl -fsSL https://raw.githubusercontent.com/kingcicou/create-scrum-team-workspace/v0.9.4/create.sh) my-project --type=new --preset=tech
```

### 方式二：PowerShell 一键执行（Windows）

```powershell
irm https://raw.githubusercontent.com/kingcicou/create-scrum-team-workspace/v0.9.4/create.ps1 | iex
```
...
npx -y github:kingcicou/create-scrum-team-workspace#v0.9.4 my-project

# 仅预览不写盘
npx -y github:kingcicou/create-scrum-team-workspace#v0.9.4 my-project --dry-run

# 交互式
npx -y github:kingcicou/create-scrum-team-workspace#v0.9.4 --interactive
```
...
- SM 对角色手册签核负责编排和闭环，内置首签、重签、误派纠偏和完成通知；
  成员只签本人行，审计器直接输出范围、待处理人和 SM 下一动作。
