...
```bash
bash <(curl -fsSL https://raw.githubusercontent.com/kingcicou/create-scrum-team-workspace/v0.9.5/create.sh) my-project
```

可叠加任意 CLI 选项：

```bash
bash <(curl -fsSL https://raw.githubusercontent.com/kingcicou/create-scrum-team-workspace/v0.9.5/create.sh) my-project --type=new --preset=tech
```

### 方式二：PowerShell 一键执行（Windows）

```powershell
irm https://raw.githubusercontent.com/kingcicou/create-scrum-team-workspace/v0.9.5/create.ps1 | iex
```
...
npx -y github:kingcicou/create-scrum-team-workspace#v0.9.5 my-project

# 仅预览不写盘
npx -y github:kingcicou/create-scrum-team-workspace#v0.9.5 my-project --dry-run

# 交互式
npx -y github:kingcicou/create-scrum-team-workspace#v0.9.5 --interactive
```
...
- SM 对角色手册签核负责编排和闭环，内置首签、重签、误派纠偏和完成通知；
  成员只追加本人签核事件，审计器直接输出范围、待处理人和 SM 下一动作。
- Sprint 经验回流采用“来源、L2 知识、L3 操作、验证、发布、项目闭环”六层
  DoD，避免只修项目或模板功能却遗漏知识传承。
- 角色签核采用 Change/Campaign/Event 事件模型，支持非连续版本补签、全量重
  基线和 Git 自动取证，同时分开报告当前有效性与历史完整性。
